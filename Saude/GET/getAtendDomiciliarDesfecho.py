...
query = text("select protocolo from cnv_antenddesfecho "
             "where protocolo is not null and id_gerado is null and sequenc > 538541 group by protocolo")
               ...
               ## json_string = response_data.decode('utf-8')
                ...
                if status_lote == "EXECUTADO" or  status_lote == "EXECUTADO_PARCIALMENTE":
                    for item in retornos:
                        ...
                        if status == "ERRO":
                            stmt = text(
                                f"UPDATE cnv_antenddesfecho SET mensagemerro = '{mensaErro}' WHERE  sequenc = {id_integracao}")
                            session.execute(stmt)
                            session.commit()
