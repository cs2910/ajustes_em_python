...
query = text("select protocolocaval from cnv_elegibilidade where protocolocaval is not null group by protocolocaval")
               ...
               ## json_string = response_data.decode('utf-8')
                ...
                if status_lote == "EXECUTADO" or  status_lote == "EXECUTADO_PARCIALMENTE":
                    for item in retornos:
                        ...
                        if status == "SUCESSO":
                            stmt = text(f"insert into cnv_elegebilidadecondaval(id_atend,id_condaval,mensagemerro, protocolo) "
                                        f"values({id_integracao},{id_gerado},{mensaErro},'{f_token_retorno}')")
                        ...
                        if status == "ERRO":
                            stmt = text(
                                f"insert into cnv_elegebilidadecondaval(id_atend,id_condaval,mensagemerro, protocolo) "
                                f"values({id_integracao},{id_gerado},{mensaErro},'{f_token_retorno}')")
                            session.execute(stmt)
                            session.commit()
