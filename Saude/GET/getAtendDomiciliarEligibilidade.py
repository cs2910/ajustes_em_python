...
query = text("select protocolo from cnv_elegibilidade where protocolo is not null "
             "and id_gerado is null and sequenc > 133501"
             "group by protocolo")
...
for tabela in resultados:
               ...
               ## json_string = response_data.decode('utf-8')
                ...
                if status_lote == "EXECUTADO" or  status_lote == "EXECUTADO_PARCIALMENTE":
                    for item in retornos:
                        ...
                        if status == "SUCESSO":
                            stmt = text(
                                f"UPDATE cnv_elegibilidade SET id_gerado = {id_gerado}, mensagemerro = null WHERE  sequenc = {id_integracao}")
                            session.execute(stmt)
                            session.commit()
                        if status == "ERRO":
                            stmt = text(
                                f"UPDATE cnv_elegibilidade SET mensagemerro = '{mensaErro}' WHERE  sequenc = {id_integracao}")
                            session.execute(stmt)
                            session.commit()
    ...
    query = text(f"select id_gerado from cnv_elegibilidade where protocolo = '{tabela.protocolo}'")
    ...
    for tabelaId in resultadoId:
        ...
        if f_token_id is not None:
            ...
            if status == 200:
                ...
                stmt = text(
                    f"UPDATE cnv_elegibilidade SET codigo = {f_token_codigo} WHERE  id_gerado = {tabelaId.id_gerado}")
                session.execute(stmt)
                session.commit()