...
query = text("select protocolo from cnv_atendprofissional "
               ...
               ## json_string = response_data.decode('utf-8')
                ...
                if status_lote == "EXECUTADO" or  status_lote == "EXECUTADO_PARCIALMENTE" or status_lote == "ERRO":
                    for item in retornos:
                        ...
                        if status == "SUCESSO":
                            stmt = text(
                                f"UPDATE cnv_atendprofissional SET id_gerado = {id_gerado}, menssagemerro = null WHERE  sequenc = {id_integracao}")
                            ##print(stmt)
                            session.execute(stmt)
                            session.commit()
                        if status == "ERRO":
                            stmt = text(
                                f"UPDATE cnv_atendprofissional SET menssagemerro = '{mensaErro}' WHERE  sequenc = {id_integracao}")
                            session.execute(stmt)
                            session.commit()
