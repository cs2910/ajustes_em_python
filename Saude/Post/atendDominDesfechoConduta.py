...
#### Linka
urlPost = 'https://saude.betha.cloud/api-saude-service-layer/v2/api/atendimentoDesfechoCondutas'
#### Token da Endidade
...
query = text("select d.id_gerado as iddesfecho,c.sequenc,desfecho,id_profissional,id_cbo,id_especialidade,c.codigo,c.id_gerado "
             ...
             "c.sequenc = d.sequenc  "
             "where desfecho in (5,6,7) and d.id_gerado is not null")
...
for tabela in resultados:
    ...
    if tabela.desfecho == 5:
        iddesfecho = 5125
    elif tabela.desfecho == 6:
        iddesfecho = 5126
    elif tabela.desfecho == 7:
        iddesfecho = 5127


    data.append({
        "idIntegracao": f'{tabela.sequenc}',
        "atendimentoDesfechoConduta": {
            "atendimentoDesfecho": {
                "id": tabela.iddesfecho
            },
    ...
    ##        print("entrou")
            ...
            stmt = text(f"UPDATE cnv_antenddesfecho SET protocoloconduta = '{f_token_retorno}' WHERE sequenc IN ({lista_sequenc_formatada})")
...
if data:
    ...
    if status == 200:
        ...
        lista_sequenc_formatada = ", ".join([f"'{item['idIntegracao']}'" for item in data])
        stmt = text(
            f"UPDATE cnv_antenddesfecho SET protocoloconduta = '{f_token_retorno}' WHERE sequenc IN ({lista_sequenc_formatada})")
        session.execute(stmt)
        session.commit()
    else:
        print(f'erro execução')