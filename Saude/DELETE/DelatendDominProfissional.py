...
query = text("select p.idgerado,a.sequenc,a.codigo, p.atenddomic from cnv_atenddomicprofissionalcloud p "
             "inner join cnv_id_atendom a on a.id_gerado = p.atenddomic")
...
for tabela in resultados:
    equipe = {}
    print(f'{tabela.sequenc} - {tabela.codigo} - {tabela.idgerado} - {tabela.atenddomic}')
    data.append({
        "idIntegracao": f"{tabela.sequenc}",
        "atendimentoDomiciliarProfissional": {
            "idGerado": tabela.idgerado
        }
    })
    ...
    ##        print("entrou")
            ...
            stmt = text(f"UPDATE cnv_atendprofissional SET protocolodelete = '{f_token_retorno}' WHERE sequenc IN ({lista_sequenc_formatada})")
