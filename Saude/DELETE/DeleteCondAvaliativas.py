...
query = text("select idgerado,idavaliacao, sequenc,a.codigo from cnv_delcondavaliativas ca  "
             "inner join cnv_id_atendom a on "
             "a.id_gerado = ca.idgerado where ca.protocolo is null")
...
for tabela in resultados:
    print(f'{tabela.sequenc} - {tabela.codigo} - {tabela.idgerado} - {tabela.idavaliacao}')
    data.append({
        "idIntegracao": f"{tabela.idgerado}",
        "atendimentoDomiciliarCondicaoAvaliada": {
            "idGerado": tabela.idavaliacao
        }
    })
    ...
    ##        print("entrou")
            ...
            lista_de_pares = [
                (
                    item['idIntegracao'],
                    item['atendimentoDomiciliarCondicaoAvaliada']['idGerado']
                )
                for item in data
            ]

            valores_formatados = [
                f"('{idgerado}', {idavaliacao})"
                for idgerado, idavaliacao in lista_de_pares
            ]
            valores_in_sql = ", ".join(valores_formatados)
            stmt = text(f"""
                        UPDATE cnv_delcondavaliativas 
                        SET protocolo = '{f_token_retorno}' 
                        WHERE (idgerado, idavaliacao) IN ({valores_in_sql})
                        """)
