...
'''cursor_postgres.execute(f"select c.* from central.pessoa p "
                        f"inner join tributario.contribuinte c on "
                        f"c.contribuinte_nome_pesquisa = p.pessoa_razao_social and "
                        f"c.contribuinte_pessoa_id = p.pessoa_id "
                        f"where p.pessoa_id > 43667 order by p.pessoa_id")'''

cursor_postgres.execute(f"select *  from tributario.nfse where nfse_id > 61210 and nfse_avulsa = 1")
...
print(colunas)
'''cursor_sybase.execute(f"""
    CREATE TABLE cnv_imovel (
        {', '.join([f'{coluna} VARCHAR(255)' for coluna in colunas])}
    )
""")
'''
...
cursor_sybase.execute(f"CREATE TABLE cnvv_{tabela} ({colunas_sybase})")

# Preparar a query de inserção no Sybase
valores_placeholder = ", ".join(["?" for _ in colunas])
sql_insert = f"INSERT INTO cnv_{tabela} VALUES ({valores_placeholder})"
