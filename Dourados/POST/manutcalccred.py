...
##for tabela, imovel in tabelas:
    idTabela = tabela.id
    data = [{
        "idIntegracao":f'credito {idTabela}',
        "manutCalcCreditos":{
            "idCreditoTributario": 154403,
            "idManutencaoCalculo": tabela.idcloud
        }
    }]
    ...
    if status == 200:
        ...
        '''Update do Protocolo'''
        stmt = (update(Benparcial).where(Benparcial.id == idTabela).values(protroccred=f_token_retorno))
        print(stmt)
        session.execute(stmt)  ##f_situacao = dadosGet['situacao']
        session.commit()
    else:
        '''Update do Protocolo'''
        stmt = (update(Benparcial).where(Benparcial.id == idTabela).values(protroccred='erro'))
        print(stmt)
        session.execute(stmt)  ##f_situacao = dadosGet['situacao']
        session.commit()