...
tabelas = session.query(Benparcial).where(Benparcial.id > 4,
                                          Benparcial.protrocref.isnot(None))
for tabela in tabelas:
    idTabela = tabela.id
    f_token_retorno = tabela.protrocref
    print(f_token_retorno)
    f_situacao = ""
    print(f_token_retorno)
    servico = requests.get(f'https://tributos.betha.cloud/service-layer-tributos/api/manutCalcReferentes/{f_token_retorno}',
    ...
    if status == 200:
        ...
        print(id_value)
        upd = (update(Benparcial).where(Benparcial.id == idTabela).values(idcloudref = id_value))
        print(upd)
        session.execute(upd)
        session.commit()
