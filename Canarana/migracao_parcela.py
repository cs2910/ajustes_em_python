...
cursor_postgres.execute(f"select * from tributario.reparcelamento r where r.reparc_data > '2024-04-30'")
...
cursor_postgres.execute(f"SELECT column_name, data_type FROM information_schema.columns WHERE table_name = '{tabela}' AND table_schema = 'tributario'")
...
cursor_sybase.execute(f"CREATE TABLE cnv_{tabela} ({colunas_sybase})")
